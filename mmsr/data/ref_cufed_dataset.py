...
class RefCUFEDDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            img_ref = img_ref * 255
            img_ref = Image.fromarray(cv2.cvtColor(img_ref.astype(np.uint8), cv2.COLOR_BGR2RGB))
            ...
            img_in_x2 = mmcv.imresize(img_in, (gt_w//2, gt_h//2), interpolation='bicubic')
            img_in_x4 = mmcv.imresize(img_in, (gt_w//4, gt_h//4), interpolation='bicubic')

            img_ref_x2 = mmcv.imresize(img_ref, (gt_w//2, gt_h//2), interpolation='bicubic')
            img_ref_x4 = mmcv.imresize(img_ref, (gt_w//4, gt_h//4), interpolation='bicubic')


        else:
            # for testing phase, zero padding to image pairs for same size
            img_in = mod_crop(img_in, scale*4)
            img_in_gt = img_in.copy()
            img_ref = mod_crop(img_ref, scale*4)
            ...
            if img_in_h != img_ref_h or img_in_w != img_ref_w:
                ...
                img_in_x2 = mmcv.imresize(img_in, (target_h//2, target_w//2), interpolation='bicubic')
                img_in_x4 = mmcv.imresize(img_in, (target_h//4, target_w//4), interpolation='bicubic')

                img_ref = mmcv.impad(img_ref, shape=(target_h, target_w), pad_val=0)
                img_ref_x2 = mmcv.imresize(img_ref, (target_h//2, target_w//2), interpolation='bicubic')
                img_ref_x4 = mmcv.imresize(img_ref, (target_h//4, target_w//4), interpolation='bicubic')
            else:
                img_in_x2 = mmcv.imresize(img_in, (img_in_h//2, img_in_w//2), interpolation='bicubic')
                img_in_x4 = mmcv.imresize(img_in, (img_in_h//4, img_in_w//4), interpolation='bicubic')
                img_ref_x2 = mmcv.imresize(img_ref, (img_ref_h//2, img_ref_w//2), interpolation='bicubic')
                img_ref_x4 = mmcv.imresize(img_ref, (img_ref_h//4, img_ref_w//4), interpolation='bicubic')
        ...
        img_in_pil = Image.fromarray(cv2.cvtColor(img_in_pil.astype(np.uint8), cv2.COLOR_BGR2RGB))
        img_in_pil_x2 = img_in_x2 * 255
        img_in_pil_x2 = Image.fromarray(cv2.cvtColor(img_in_pil_x2.astype(np.uint8), cv2.COLOR_BGR2RGB))
        img_in_pil_x4 = img_in_x4 * 255
        img_in_pil_x4 = Image.fromarray(cv2.cvtColor(img_in_pil_x4.astype(np.uint8), cv2.COLOR_BGR2RGB))
        

        img_ref_pil = img_ref * 255
        img_ref_pil = Image.fromarray(cv2.cvtColor(img_ref_pil.astype(np.uint8), cv2.COLOR_BGR2RGB))
        # img_ref_pil_x2 = img_ref_x2 * 255
        # img_ref_pil_x2 = Image.fromarray(cv2.cvtColor(img_ref_pil_x2.astype(np.uint8), cv2.COLOR_BGR2RGB))
        # img_ref_pil_x4 = img_ref_x4 * 255
        # img_ref_pil_x4 = Image.fromarray(cv2.cvtColor(img_ref_pil_x4.astype(np.uint8), cv2.COLOR_BGR2RGB))



        img_in_lq = img_in_pil.resize((lq_w, lq_h), Image.BICUBIC)
        img_in_lq_x2 = img_in_pil_x2.resize((lq_w//2, lq_h//2), Image.BICUBIC)
        img_in_lq_x4 = img_in_pil_x4.resize((lq_w//4, lq_h//4), Image.BICUBIC)    

        img_ref_lq = img_ref_pil.resize((lq_w, lq_h), Image.BICUBIC)
        # img_ref_lq_x2 = img_ref_pil_x2.resize((lq_w//2, lq_h//2), Image.BICUBIC)
        # img_ref_lq_x4 = img_ref_pil_x4.resize((lq_w//4, lq_h//4), Image.BICUBIC)

        # bicubic upsample LR
        img_in_up = img_in_lq.resize((gt_w, gt_h), Image.BICUBIC)
        img_in_x2_up = img_in_lq_x2.resize((gt_w//2, gt_h//2), Image.BICUBIC)
        img_in_x4_up = img_in_lq_x4.resize((gt_w//4, gt_h//4), Image.BICUBIC)

        img_ref_up = img_ref_lq.resize((gt_w, gt_h), Image.BICUBIC)
        # img_ref_x2_up = img_ref_lq_x2.resize((gt_w//2, gt_h//2), Image.BICUBIC)
        # img_ref_x4_up = img_ref_lq_x4.resize((gt_w//4, gt_h//4), Image.BICUBIC)
        ...
        img_in_x2_up = cv2.cvtColor(np.array(img_in_x2_up), cv2.COLOR_RGB2BGR)
        img_in_x2_up = img_in_x2_up.astype(np.float32) / 255.
        img_in_x4_up = cv2.cvtColor(np.array(img_in_x4_up), cv2.COLOR_RGB2BGR)
        img_in_x4_up = img_in_x4_up.astype(np.float32) / 255.
        ...
        # img_ref_x2_up = cv2.cvtColor(np.array(img_ref_x2_up), cv2.COLOR_RGB2BGR)
        # img_ref_x2_up = img_ref_x2_up.astype(np.float32) / 255.
        # img_ref_x4_up = cv2.cvtColor(np.array(img_ref_x4_up), cv2.COLOR_RGB2BGR)
        # img_ref_x4_up = img_ref_x4_up.astype(np.float32) / 255.

        # BGR to RGB, HWC to CHW, numpy to tensor
        img_in, img_in_lq, img_in_up, img_in_x2_up,img_in_x4_up, img_ref,img_ref_x2,img_ref_x4,img_ref_lq, img_ref_up = totensor(  # noqa: E501
            [img_in, img_in_lq, img_in_up,img_in_x2_up,img_in_x4_up, img_ref,img_ref_x2,img_ref_x4, img_ref_lq, img_ref_up],
            bgr2rgb=True,
            float32=True)
        ...
        return_dict = {
            ...
            'img_in_lq': img_in_lq,
            'img_in_up': img_in_up,
            'img_in_x2_up': img_in_x2_up,
            'img_in_x4_up': img_in_x4_up,
            'img_ref': img_ref,
            'img_ref_x2': img_ref_x2,
            'img_ref_x4': img_ref_x4,
